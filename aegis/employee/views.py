...
def io_state(request):
    ...
    for passer in passer_list:
        ...
        #     continue
        beacon_list = Beacon_Record.objects.filter(passer_id=passer.id, minor=1101, dt_begin__gt=dt_current)
        if len(beacon_list) == 0:
            logSend('   > timeout: {}  {}'.format(passer.id, passer.rssi_a))
            passer.rssi_a = -999
            passer.save()
        beacon_list = Beacon_Record.objects.filter(passer_id=passer.id, minor=1102, dt_begin__gt=dt_current)
        if len(beacon_list) == 0:
