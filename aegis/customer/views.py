        ...
        for employee in employees:
            ...
            if today < employee.dt_begin:
                ...
                view_employee = {'id': AES_ENCRYPT_BASE64(str(employee.id)),
                                 'name': employee.name,
                                 'pNo': phone_format(employee.pNo),
                                 'dt_begin_beacon': dt_str(employee.dt_begin_beacon, "%Y-%m-%d %H:%M:%S"),
                                 'dt_begin_touch': dt_str(employee.dt_begin_touch, "%Y-%m-%d %H:%M:%S"),
                                 'dt_end_beacon': dt_str(employee.dt_end_beacon, "%Y-%m-%d %H:%M:%S"),
                                 'dt_end_touch': dt_str(employee.dt_end_touch, "%Y-%m-%d %H:%M:%S"),
                                 'state': "",
                                 'is_not_begin': False,
                                 }
            arr_employee.append(view_employee)
